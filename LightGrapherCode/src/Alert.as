package  {
	...
	//
	/**
	 * I didn't write the Alert class, its freely available online: http://fatal-exception.co.uk/blog/?p=69
	 * Credit to dVyper, I only made minor modifications for increased flexibility and vertical display of buttons
	 */
	public class Alert {
		//
		...
		*/
		...
		public static function createButton(buttonText:String, alertOptions:AlertOptions, width:int, index:int):SimpleButton {
			var colors:Array = new Array();
			var alphas:Array = new Array(1, 1);
			var ratios:Array = new Array(0, 255);
			var gradientMatrix:Matrix = new Matrix();
			gradientMatrix.createGradientBox(width, btnHeight, Math.PI/2, 0, 0);
			//
			var text:TextField = createButtonTextField(buttonText, alertOptions, width, index);
			...
			btnUpState.graphics.drawRoundRect(0, 0, text.width, btnHeight, ellipseSize, ellipseSize);
			btnUpState.addChild(text);
			btnUpState.y = 30 * index;
			//
			...
			btnOverState.graphics.drawRoundRect(0, 0, text.width, btnHeight, ellipseSize, ellipseSize);
			btnOverState.addChild(createButtonTextField(buttonText, alertOptions, width, index));
			btnOverState.y = 30 * index;
			//
			...
			btnDownState.graphics.drawRoundRect(0, 0, text.width, btnHeight, ellipseSize, ellipseSize);
			btnDownState.addChild(createButtonTextField(buttonText, alertOptions, width, index));
			btnDownState.y = 30 * index;
			//
		...
		private static function createPrompt(alertOptions:AlertOptions):Sprite {
			...
			var myWidth:int = textField.width+50;
			var myHeight:int = alertOptions.buttons.length * 40 + 65 + textField.height;//textField.height+65;
			...
			textField.y = 20;//(promptBackground.height/2)-(textField.height/2)-20;
			//	ADD SPECIFIED BUTTONS TO THE PROMPT
			var alertButtons:Array = new Array();
			var k:int = 0;
			for (var n:int; n < alertOptions.buttons.length; n++) {
				alertButtons.push(createButton(alertOptions.buttons[n], alertOptions, promptBackground.width, k));
				k++;
			}
			...
			for (var i:int; i < alertButtons.length; i++) {
				alertButtons[i].width += 15;
				alertButtons[i].x = actualPrompt.width / 2 - alertButtons[i].width/2;
				alertButtons[i].y = textField.height + 40 * k ;
		...
		//
		private static function createButtonTextField(Text:String, alertOptions:AlertOptions, width:int, index:int):TextField {
			var myTextField:TextField = new TextField();
			myTextField.textColor = alertOptions.textColour;
			myTextField.selectable = false;
			myTextField.width = width + 15;// btnWidth + 30;
			...
			myTextField.x = 0;//(width / 2) - (myTextField.width / 2);
			myTextField.y = 0;//(40 * index + 65);
